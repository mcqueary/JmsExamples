...
public class JmsEchoService implements ExceptionListener, MessageListener {
	...
	public JmsEchoService(String[] args) {
		...
		Properties props = readPropertyFileFromClasspath(propFileName);
		if (props != null)
			ctx = getInitialContext(props);
		
		// Initialize globals for the banner
		providerContextFactory = props
		...
		try {
			...
			System.err.println("InitialContextFactory........ "
			...
			System.err
					.println("Listening for requests on destination: " + name);
	...
	private InitialContext getInitialContext(Properties p) {
		// Most implementations should correctly/automatically
		// load jndi.prooperties from the classpath, but a small
		// few don't, so we're hacking this for now
		InitialContext ctx = null;
...
//		}
		...
		try {
			// TODO Change to no-arg implementation once non-compliant 
	...
	void usage() {
		...
		System.err.print("\n");
		System.err
				.print(" -server   <server URL> - JMS broker URL\n");
		System.err
				.print(" -cf       <class name> - JMS provider's initial ContextFactory\n");
		System.err
				.print(" -user     <user name>  - user name, default is null\n");
		System.err
				.print(" -password <password>   - password, default is null\n");
		System.err
				.print(" -dest     <dest name>  - Destination name, default is \"queue.sample\"\n");
		System.exit(0);
	}
	...
	void parseArgs(String[] args) {
		...
		while (i < args.length) {
			...
			if (args[i].compareTo("-cf") == 0) {
				...
				i += 2;
			} else if (args[i].compareTo("-dest") == 0) {
				if ((i + 1) >= args.length)
					usage();
				name = args[i + 1];
				i += 2;
	...
	public void onMessage(Message msg) {
		...
		try {
			...
			System.err.println("Received request ID:" + requestID + " Body:\""
					+ requestMsg.getText() + "\"");
